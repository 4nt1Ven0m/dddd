...
function library:CreateWindow(name, size, hidebutton)
    ...
    function window:CreateTab(name)
        ...
        function tab:CreateSector(name,side)
           ...
           -- Assuming 'window', 'sector', 'updateevent', 'library', 'textservice', 'camera', 'mouse', and 'uis' are defined elsewhere
-- and have the necessary properties and methods.  Also assumes a theme with properties like 'font',
-- 'itemscolor', 'outlinecolor', 'outlinecolor2', 'accentcolor', and 'accentcolor2'.

sector:AddSlider = function(text, min, default, max, decimals, callback, flag)
    local slider = { }
    slider.text = text or ""
    slider.callback = callback or function(value) end
    slider.min = min or 0
    ...
    slider.MainBack.Size = UDim2.fromOffset(sector.Main.Size.X.Offset - 12, 35) -- Increased height for tooltip
    ...
    -- Main Slider Button (for interaction)
    ...
    -- Slider Outline/Styling
    local function createOutline(parent, offset, color)
        local outline = Instance.new("Frame", parent)
        outline.Name = "blackline"
        outline.ZIndex = 4
        outline.Size = slider.Main.Size + UDim2.fromOffset(offset, offset)
        outline.BorderSizePixel = 0
        outline.BackgroundColor3 = color
        outline.Position = UDim2.fromOffset(-offset/2, -offset/2)
        return outline
    end

    slider.BlackOutline2 = createOutline(slider.Main, 6, window.theme.outlinecolor2)
    slider.Outline = createOutline(slider.Main, 4, window.theme.outlinecolor)
    slider.BlackOutline = createOutline(slider.Main, 2, window.theme.outlinecolor2)
    ...
    -- Slide Bar (Represents the value)
    slider.SlideBar = Instance.new("Frame", slider.Main)
    ...
    -- Tooltip (Initial setup - hidden)
    slider.Tooltip = Instance.new("Frame", slider.MainBack)
    slider.Tooltip.Name = "Tooltip"
    slider.Tooltip.Size = UDim2.fromOffset(100, 20) -- Initial size (adjust as needed)
    slider.Tooltip.Position = UDim2.fromOffset(0, -20) -- Position above the slider
    slider.Tooltip.BackgroundColor3 = Color3.new(0, 0, 0) -- Black background
    slider.Tooltip.BackgroundTransparency = 0.8
    slider.Tooltip.BorderSizePixel = 0
    slider.Tooltip.Visible = false  -- Initially hidden
    slider.Tooltip.ZIndex = 10

    slider.TooltipText = Instance.new("TextLabel", slider.Tooltip)
    slider.TooltipText.Name = "TooltipText"
    slider.TooltipText.Size = UDim2.fromScale(1, 1)
    slider.TooltipText.BackgroundTransparency = 1
    slider.TooltipText.TextColor3 = Color3.new(1, 1, 1) -- White text
    slider.TooltipText.Font = window.theme.font -- Use theme font
    slider.TooltipText.TextSize = 14
    slider.TooltipText.TextXAlignment = Enum.TextXAlignment.Center
    slider.TooltipText.TextYAlignment = Enum.TextYAlignment.Center
    slider.TooltipText.TextStrokeTransparency = 1

    -- Function to update the tooltip
    local function updateTooltip(value)
        if not slider.Tooltip.Visible then return end  -- Only update if visible
        local tooltipText = tostring(value)
        slider.TooltipText.Text = tooltipText
        -- Resize tooltip to fit text
        local textSize = textservice:GetTextSize(tooltipText, slider.TooltipText.TextSize, slider.TooltipText.Font, Vector2.new(math.huge, math.huge))
        slider.Tooltip.Size = UDim2.fromOffset(textSize.X + 10, 20) -- Add padding

        -- Reposition the tooltip above the slider, centered horizontally
        slider.Tooltip.Position = UDim2.new(0, (slider.Main.AbsoluteSize.X - textSize.X - 10) / 2, 0, -20) -- Centered above
    end

    -- Show/Hide Tooltip, Update Tooltip
    ...
    -- Tooltip events for slider.Main
    slider.Main.MouseEnter:Connect(showTooltip)
    slider.Main.MouseLeave:Connect(hideTooltip)

    -- Tooltip events for slider.SlideBar (in case mouse is already on the bar)
    slider.SlideBar.MouseEnter:Connect(showTooltip)
    slider.SlideBar.MouseLeave:Connect(hideTooltip)
    ...
    return slider
        ...
        function tab:CreatePlayerlist(name)
            ...
            function list:AddPlayer(Player)
                ...
                player.Label = Instance.new("TextLabel", player.Main)
                player.Label.Name = "NameLabel"
                player.Label.Text = Player.Name
                player.Label.TextColor3 = Color3.new(1,1,2552/255) -- or a suitable color.
                player.Label.TextStrokeTransparency = 1
                player.Label.Font = window.theme.font
                player.Label.TextSize = 12
                player.Label.BackgroundTransparency = 1
                player.Label.Size = UDim2.fromScale(1,1)
                player.Label.TextXAlignment = Enum.TextXAlignment.Left
                player.Label.TextPadding =  UDim2.new(0, 6, 0, 0)
            ...
            function list:RemovePlayer(playerName)
                local p = list.Items:FindFirstChild(playerName)
                if p then
                    ...
                    p:Destroy() -- Destroy the player frame, not just remove.
                    list.Items.CanvasSize = UDim2.fromOffset(list.Items.AbsoluteSize.X, (#list.items * 20)) -- Recalculate canvas size.
                end
            end
            ...
            -- Function to handle player removal from the list
            game:GetService("Players").PlayerRemoving:Connect(function(Player)
                list:RemovePlayer(Player.Name)
            end)

            -- Update the player list when the game starts by adding all existing players
            function list:UpdatePlayerList()
                for _, player in pairs(game:GetService("Players"):GetPlayers()) do
                    if not table.find(list.items, player) then
                        list:AddPlayer(player)
                    end
                end
            end

            -- Initial call to update the player list
            list:UpdatePlayerList()

            -- Optional functionality: Clear the list (can be used to refresh the player list)
            function list:ClearPlayers()
                for _, player in pairs(list.items) do
                    list:RemovePlayer(player.Name)
                end
            end
        ...
        -- Function to create and hold multiple player lists within the main tab
        function tab:CreatePlayerLists(names)
            local playerLists = {}
            for _, name in ipairs(names) do
                table.insert(playerLists, tab:CreatePlayerlist(name))
            end
            return playerLists
        end
